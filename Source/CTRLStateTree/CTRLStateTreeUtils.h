...
#include "CTRLStateTree.h"

#include "Components/StateTreeComponent.h"

#include "Kismet/BlueprintFunctionLibrary.h"

#include "CTRLStateTreeUtils.generated.h"
...
UCLASS(DisplayName = "StateTree Utils [CTRL]", ClassGroup=(UST))
class CTRLSTATETREE_API UCTRLStateTreeUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	static bool GetStateName(UStateTree const* Tree, FStateTreeStateHandle const& StateHandle, FName& OutName)
	{
		...
		{
			CTRLST_LOG(Warning, TEXT("StateTreeUtils::GetStateName: StateHandle is not valid."));
			return false;
		}
		if (!IsValid(Tree))
		{
			CTRLST_LOG(Warning, TEXT("StateTreeUtils::GetStateName: Tree is not valid."));
			return false;
		}
		auto const State = Tree->GetStateFromHandle(StateHandle);
		if (!State)
		{
			CTRLST_LOG(Warning, TEXT("StateTreeUtils::GetStateName: Could not find state from handle"));
			return false;
		}
		OutName = State->Name;
		return true;
